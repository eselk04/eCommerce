...
public class Mapper : eCommerce.Application.Interfaces.AutoMapper.IMapper
{
    ...
    protected void Config<TDestination, TSource>(int depth = 5, string? ignore = null)
    {
        ...
        if(typePairs.Any(a => a.SourceType == typePair.SourceType && a.DestinationType == typePair.DestinationType && ignore is null) )
            return;
         ...
         foreach (var item in typePairs)
         {
             if (ignore is not null)
             {
                 cfg.CreateMap(item.SourceType, item.DestinationType).MaxDepth(depth)
                     .ForMember(ignore, opt => opt.Ignore()).ReverseMap();
             }
             else
             {
